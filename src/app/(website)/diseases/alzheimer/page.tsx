import React from "react";
import Breadcrumb from "@/components/Breadcrumb";
import EnquiryFormSide from "@/components/EnquiryFormSide";
import { Metadata } from "next";

export const metadata: Metadata = {
    title: "Ayurvedic Alzheimer’s Disease Treatment | Atharv Veda",
    description: "Holistic Ayurvedic treatment for Alzheimer’s disease focusing on root causes and natural symptom management.",
    alternates: {
        canonical: "/diseases/alzheimer",
    },
};

export default function AlzheimerPage() {
    return (
        <main>
            <Breadcrumb title="Alzheimer’s Disease" />

            {/* Hero Section */}
            <div className="ayur-kidney-section" style={{ background: "#071818", padding: "60px 0 40px 0" }}>
                <div className="container">
                    <div className="row align-items-center">
                        <div className="col-lg-7 col-md-12 mb-4 mb-lg-0">
                            <div className="text-white" style={{ maxWidth: "600px" }}>
                                <h1 style={{ fontSize: "3rem", fontWeight: 800 }}>
                                    Ayurvedic <span style={{ color: "#ffb300", fontSize: "larger" }}>Alzheimer’s Disease</span><br />
                                    Treatment
                                </h1>
                                <div style={{ margin: "32px 0 24px 0" }}>
                                    <span style={{ background: "#ff5722", color: "#fff", fontSize: "2.5rem", fontWeight: 700, padding: "10px 32px", borderRadius: "4px", display: "inline-block" }}>ATHARVEDA</span>
                                </div>
                                <div style={{ fontSize: "2rem", color: "#fff", fontWeight: 400 }}>100% Ayurvedic Treatment</div>
                                <div style={{ marginTop: "40px" }}>
                                    <img src="/assets/images/alzheimers.webp" alt="Alzheimer" style={{ maxWidth: "180px", display: "block" }} />
                                </div>
                            </div>
                        </div>
                        <div className="col-lg-5 col-md-12">
                            <EnquiryFormSide disease="Alzheimer’s Disease" />
                        </div>
                    </div>
                </div>
            </div>

            {/* What is Alzheimer's */}
            <div className="ayur-kidney-info-section" style={{ background: "linear-gradient(180deg, #f8f9fa 0%, #fff 100%)", padding: "60px 0 40px 0", borderRadius: "0 0 24px 24px", marginBottom: "40px" }}>
                <div className="container">
                    <div className="row align-items-center">
                        <div className="col-lg-7 col-md-12 mb-4 mb-lg-0">
                            <h2 style={{ fontSize: "3rem", fontWeight: 800, color: "#00423b", marginBottom: "24px" }}>
                                What Is <span style={{ color: "#ffb300", fontSize: "larger" }}>Alzheimer’s Disease?</span>
                            </h2>
                            <p style={{ fontSize: "1.25rem", color: "#18423b", marginBottom: "24px", maxWidth: "800px" }}>
                                Alzheimer’s disease is the most common type of dementia, a condition that slowly affects the brain’s ability to think, remember, and communicate. It begins with mild memory loss and gradually worsens, making it hard for a person to carry out daily tasks. As the disease progresses, the person may lose the ability to respond to their environment or hold a conversation.<br /><br />
                                Ayurvedic treatment for Alzheimer’s disease focuses on addressing the root causes and managing the symptoms naturally to improve quality of life.
                            </p>
                            <button className="btn" style={{ background: "#00423b", color: "#fff", fontSize: "1rem", fontWeight: 500, padding: "10px 32px", borderRadius: "32px" }}>Book Consultation</button>
                        </div>
                        <div className="col-lg-5 col-md-12 text-center">
                            <div style={{ background: "#fff", borderRadius: "24px", boxShadow: "0 8px 32px rgba(0,0,0,0.08)", padding: "16px 0 0 0", display: "inline-block" }}>
                                <img src="/assets/images/alzheimers.webp" alt="Alzheimer" style={{ maxWidth: "100%", borderRadius: "16px", boxShadow: "0 4px 24px rgba(255,179,0,0.15)" }} />
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            {/* Causes */}
            <div className="ayur-kidney-treatment-info-section" style={{ background: "linear-gradient(90deg, #f8f9fa 60%, #fffbe7 100%)", padding: "60px 0 40px 0", marginBottom: "40px", borderRadius: "24px" }}>
                <div className="container">
                    <div className="row align-items-center">
                        <div className="col-lg-5 col-md-12 mb-4 mb-lg-0 text-center">
                            <img src="/assets/images/alzheimers.webp" alt="Alzheimer" style={{ maxWidth: "340px", borderRadius: "16px", boxShadow: "0 8px 32px rgba(0,0,0,0.10)", background: "#fff", padding: "16px" }} />
                        </div>
                        <div className="col-lg-7 col-md-12">
                            <h2 style={{ fontSize: "2.8rem", fontWeight: 800, color: "#00423b", marginBottom: "18px" }}>
                                Causes of <span style={{ color: "#ffb300", fontSize: "larger" }}>Alzheimer’s Disease</span>
                            </h2>
                            <p style={{ fontSize: "1.25rem", color: "#18423b", marginBottom: "18px" }}>
                                To choose the most effective Ayurvedic medicine for Alzheimer’s, understanding the underlying causes is important. One of the major causes is the abnormal build-up of proteins in and around brain cells...
                            </p>
                            <ul style={{ fontSize: "1.15rem", color: "#18423b", marginBottom: "18px", listStyleType: "disc", paddingLeft: "24px" }}>
                                <li><b>Amyloid:</b> Forms plaques around the brain cells</li>
                                <li><b>Tau:</b> Causes tangles inside brain tissue</li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>

            {/* Symptoms and Complications */}
            <div className="ayur-mnd-types-section" style={{ background: "linear-gradient(90deg, #fffbe7 60%, #f8f9fa 100%)", padding: "60px 0 40px 0", marginBottom: "40px", borderRadius: "24px" }}>
                <div className="container">
                    <div className="row align-items-center">
                        <div className="col-lg-7 col-md-12 mb-4 mb-lg-0">
                            <h2 style={{ fontSize: "2.5rem", fontWeight: 800, color: "#00423b", marginBottom: "18px" }}>
                                Signs and Symptoms
                            </h2>
                            <ul style={{ fontSize: "1.15rem", color: "#18423b", marginBottom: "18px", listStyleType: "disc", paddingLeft: "24px" }}>
                                <li>Memory loss and poor judgment</li>
                                <li>Repeating the same conversations or questions</li>
                                <li>Aphasia (difficulty speaking or understanding speech)</li>
                                <li>Confusion and mood swings</li>
                                <li>Disturbed sleep cycle</li>
                                <li>Difficulty swallowing (dysphagia)</li>
                            </ul>
                        </div>
                        <div className="col-lg-5 col-md-12">
                            <div style={{ background: "#fff", borderRadius: "16px", boxShadow: "0 8px 32px rgba(0,0,0,0.08)", padding: "28px 24px", marginBottom: "24px" }}>
                                <h3 style={{ fontSize: "1.3rem", color: "#ff5722", fontWeight: 700, marginBottom: "12px" }}>How Ayurveda Helps</h3>
                                <ul style={{ fontSize: "1.05rem", color: "#18423b", marginBottom: "0", listStyleType: "disc", paddingLeft: "24px" }}>
                                    <li>Herbal remedies to improve memory, sleep, and mood</li>
                                    <li>Therapies to restore brain balance</li>
                                    <li>Sativk diet rich in brain-boosting foods</li>
                                    <li>Panchakarma detoxification</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </main>
    );
}
